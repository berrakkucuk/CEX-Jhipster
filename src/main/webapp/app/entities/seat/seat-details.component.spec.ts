/* tslint:disable max-line-length */
import { vitest } from 'vitest';
import { shallowMount, type MountingOptions } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import SeatDetails from './seat-details.vue';
import SeatService from './seat.service';
import AlertService from '@/shared/alert/alert.service';

type SeatDetailsComponentType = InstanceType<typeof SeatDetails>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const seatSample = { id: 123 };

describe('Component Tests', () => {
  let alertService: AlertService;

  afterEach(() => {
    vitest.resetAllMocks();
  });

  describe('Seat Management Detail Component', () => {
    let seatServiceStub: SinonStubbedInstance<SeatService>;
    let mountOptions: MountingOptions<SeatDetailsComponentType>['global'];

    beforeEach(() => {
      route = {};
      seatServiceStub = sinon.createStubInstance<SeatService>(SeatService);

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        bvToast: {
          toast: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'router-link': true,
        },
        provide: {
          alertService,
          seatService: () => seatServiceStub,
        },
      };
    });

    describe('Navigate to details', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        seatServiceStub.find.resolves(seatSample);
        route = {
          params: {
            seatId: '' + 123,
          },
        };
        const wrapper = shallowMount(SeatDetails, { global: mountOptions });
        const comp = wrapper.vm;
        // WHEN
        await comp.$nextTick();

        // THEN
        expect(comp.seat).toMatchObject(seatSample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        seatServiceStub.find.resolves(seatSample);
        const wrapper = shallowMount(SeatDetails, { global: mountOptions });
        const comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
